...
- `specialist_route`: use a controlled modality only when it is central. Local builds and tests do not become `code_execution`. For high-impact authentication, cryptographic, or key-management work, `security_review` may accompany `CRITICAL`; for sensitive spreadsheets or account browser actions, retain `CRITICAL` and record the corresponding modality.
